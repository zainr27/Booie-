...
const LoanSimulator: React.FC<LoanSimulatorProps> = ({
  ...
  const handleGrowthRateChange = (year: number, newGrowthRate: number) => {
    ...
    if (rowIndex > 0) {
      ...
      updatedProjections[rowIndex].booiePayment = calculateBooieMonthlyPayment(updatedProjections[rowIndex].income, loanParams.booieRate);
      ...
      for (let i = rowIndex + 1; i < updatedProjections.length; i++) {
        ...
        updatedProjections[i].booiePayment = calculateBooieMonthlyPayment(updatedProjections[i].income, loanParams.booieRate);
      }
    }
    setIncomeProjections(updatedProjections);
  };
  ...
  const handleLoanParamChange = (paramName: keyof LoanParameters, value: number) => {
    setLoanParams(prev => ({
      ...prev,
      [paramName]: value
    }));
  ...
  return <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <Card className="p-4 md:w-[280px] shrink-0 bg-gray-900">
            ...
            <TableHeader>
              ...
              <TableRow>
                ...
                <TableHead className="bg-gray-900">
                  <Input type="number" min="0.01" max="0.5" step="0.001" value={loanParams.booieRate} onChange={e => handleLoanParamChange('booieRate', parseFloat(e.target.value))} className="h-8 text-sm" aria-label="Booie rate" />
                  <div className="text-xs text-center mt-1">{formatPercentage(loanParams.booieRate)} rate</div>
                </TableHead>
                <TableHead className="bg-gray-900">
                  <Input type="number" value={loanParams.federalRate} min="0.01" max="0.5" step="0.001" onChange={e => handleLoanParamChange('federalRate', parseFloat(e.target.value))} className="h-8 text-sm text-red-600" aria-label="Federal loan interest" />
                ...
                <TableHead className="bg-slate-900">
                  <Input type="number" value={loanParams.privateRate} min="0.01" max="0.5" step="0.001" onChange={e => handleLoanParamChange('privateRate', parseFloat(e.target.value))} className="h-8 text-sm text-red-600" aria-label="Private loan interest" />
              ...
              <TableRow>
                ...
                <TableHead className="bg-gray-900">
                  <Input type="number" value={loanParams.privateTerm} min="1" max="30" step="1" onChange={e => handleLoanParamChange('privateTerm', parseInt(e.target.value))} className="h-8 text-sm text-red-600" aria-label="Private loan term" />
                  <div className="text-xs text-center mt-1 text-red-600">{loanParams.privateTerm} years</div>
                </TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {incomeProjections.map(row => <TableRow key={row.year}>
                  <TableCell>{row.year}</TableCell>
                  <TableCell>
                    <Input type="number" value={row.income} min="0" step="1000" onChange={e => handleIncomeChange(row.year, parseInt(e.target.value))} className="w-24 h-8 text-sm text-red-600" aria-label={`Income for year ${row.year}`} />
                  </TableCell>
                  <TableCell>
                    {row.year === 1 ? '-' : <Input type="number" value={row.growthRate} min="-0.5" max="0.5" step="0.005" onChange={e => handleGrowthRateChange(row.year, parseFloat(e.target.value))} className="w-16 h-8 text-sm text-red-600" aria-label={`Growth rate for year ${row.year}`} />}
                  </TableCell>
                  <TableCell>
      ...
      <div className="sticky bottom-4 md:bottom-auto md:flex md:justify-end mt-4">
        <Button className="w-full md:w-auto bg-booie-600 hover:bg-booie-700" size="lg" onClick={handleApplyClick}>
